...
export class TrainingScene extends Phaser.Scene {
  ...
  // Touch gear ROCKER: two big paddles (G＋/G−) docked at the RIGHT EDGE with the
  // current gear displayed between them. No dragging, no mid-field widget - the
  // old lever was hard to use, hard to read, and sat in the play space.
  private rockerGfx?: Phaser.GameObjects.Graphics;
  private rockerText?: Phaser.GameObjects.Text;
  ...
  private setupTouch() {
    ...
    void hold;
    // FOUR buttons in a thumb arc (was six): 弱 at the corner rest position, 強
    // beside it, 投 outer, and ONE 必殺 button above - the held stick direction
    // picks the special (neutral=波動枠, ↑=昇竜枠, ↓=超必), same as assist mode.
    press(GAME_WIDTH - 34, GAME_HEIGHT - 38, 15, '弱', 0xffdd66, () => { this.touchPress.light = true; });
    ...
    this.buildGearRocker();
  }

  /** The gear rocker: two large paddles docked at the right edge with the live
  ...
  private buildGearRocker() {
    ...
    const draw = (g: Phaser.GameObjects.Graphics) => {
      ...
      g.lineStyle(1.5, 0x66ddaa, 0.55); g.strokeRoundedRect(x0, top, w, ph * 2 + mid, 8);
      g.lineStyle(1, 0x66ddaa, 0.3); g.lineBetween(x0 + 4, top + ph, x0 + w - 4, top + ph);
      g.lineBetween(x0 + 4, top + ph + mid, x0 + w - 4, top + ph + mid);
    };
    draw(this.rockerGfx);
    this.add.text(x0 + w / 2, top + ph / 2, 'G＋', { fontFamily: PIXEL_FONT, fontSize: '10px', color: '#8fd6b0', fontStyle: 'bold' })
      .setOrigin(0.5).setResolution(2);
    this.add.text(x0 + w / 2, top + ph + mid + ph / 2, 'G−', { fontFamily: PIXEL_FONT, fontSize: '10px', color: '#8fd6b0', fontStyle: 'bold' })
      .setOrigin(0.5).setResolution(2);
    this.rockerText = this.add.text(x0 + w / 2, top + ph + mid / 2, '1', {
      fontFamily: PIXEL_FONT, fontSize: '10px', color: '#ffffff', fontStyle: 'bold',
    }).setOrigin(0.5).setResolution(2);
    const upZone = this.add.zone(x0 + w / 2, top + ph / 2, w + 6, ph + 4).setInteractive();
    upZone.on('pointerdown', () => { this.touchPress.gearUp = true; });
    const dnZone = this.add.zone(x0 + w / 2, top + ph + mid + ph / 2, w + 6, ph + 4).setInteractive();
    dnZone.on('pointerdown', () => { this.touchPress.gearDown = true; });
  }

  /** Live rocker readout: the gear number, heat-tinted; '!!' during overheat;
   * green while the perfect-shift (double-tap) window is open. */
  private updateGearRocker() {
    const t = this.rockerText;
    if (!t) return;
    const f = this.engine.p1;
    if (f.clutchOpen > 0) { t.setText(`${f.gear}`); t.setColor('#7affc8'); return; }
    if (f.overheated) { t.setText('!!'); t.setColor(this.time.now % 300 < 150 ? '#ff5533' : '#ff9988'); return; }
    t.setText(`${f.gear}`);
    const heat = Math.min(1, f.heat / 100);
    t.setColor(heat > 0.7 ? '#ff8866' : heat > 0.4 ? '#ffcc88' : '#ffffff');
  }
